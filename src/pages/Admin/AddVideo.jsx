...
export const AddVideo = () => {
  ...
  // Inisialisasi dataAllChapter dengan useMemo
  const dataAllChapter = useMemo(() => getAllChapter?.data.chapter || [], [getAllChapter?.data.chapter]);

  // Contoh penggunaan useMemo dan useEffect
  useEffect(() => {
    // Gunakan dataAllChapter di sini
    // Contoh: console.log(dataAllChapter);
    // Contoh logika efek samping
    // Misalnya, menjalankan sesuatu ketika dataAllChapter berubah
  }, [dataAllChapter]);

  useEffect(() => {
    const filterAllChapter = dataAllChapter.filter((value) => {
      return value.chapter_id === getIdChapter; // Mengembalikan true atau false
    });
  ...
  // POST CHAPTER
  const { mutate: postChapter, isSuccess: successPostChapter } = usePostChapter();
  ...
  useEffect(() => {
    ...
    // Menggunakan filter() dengan benar
    const filterAllChapter = dataAllChapter.filter((value) => {
      return value.chapter_id === getIdChapter;
    });
  ...
  // Menggunakan useMemo untuk menginisialisasi dataVideoChapter
  const dataVideoChapter = useMemo(() => {
    return getVideoPerChapter?.data?.video || [];
  }, [getVideoPerChapter]); // Tambahkan dependensi yang diperlukan di sini

  // Menggunakan useEffect dengan dependensi yang sesuai
  useEffect(() => {
    // Logika useEffect
  }, [dataVideoChapter]); // Tambahkan dependensi yang sesuai di sini
  ...
  const handleVideoPerChapter = (idChapter) => {
    ...
    if (successVideoChapter) {
      const videoChapter = dataVideoChapter?.filter((value) => {
        ...
        return value.chapter_id === idChapter && value.video_id !== 201;
      });
  ...
  // Menggunakan useMemo untuk menginisialisasi dataVideoById
  const dataVideoById = useMemo(() => {
    return getVideoById?.data.video || [];
  }, [getVideoById]); // Tambahkan dependensi yang diperlukan di sini
  ...
  const { mutate: postVideo, data: getDataVideo, isSuccess: successDataVideo } = usePostVideo();
      ...
      <LayoutAdmin>
        ...
        <div className="text-sm">
          <div className="flex items-center justify-between">
            <div onClick={() => navigate("/admin/kelolakelas")} className="flex items-center gap-2 cursor-pointer hover:text-gray-400">
              <IoIosArrowRoundBack />
              <div>Kembali</div>
            </div>
            <div className="flex items-center gap-1.5">
              <div onClick={() => navigate("/admin/kelolakelas")} className="cursor-pointer hover:text-gray-400">
                Kelola Kelas
              </div>
              <div>/</div>
          ...
          <div className="w-full flex gap-4 mt-4">
            <div className={`${getCloseAddEdit ? "w-1/4" : "w-1/2"}`}>
              <div className="bg-primary text-white rounded ps-2 text-lg font-semibold">Chapter</div>
              <div className="mt-1 font-medium text-sm px-1">
                <div className={`text-center py-2 ${dataChapter.length === 0 ? "block" : "hidden"}`}>Belum ada video.</div>
                    ...
                    <div key={index} className="py-2 border-b border-gray-300">
                      {getEditChapter && getIdChapter === value.chapter_id ? (
                        <div className="relative flex items-center border-b border-gray-300">
                          <input type="text" placeholder="new chapter" className="py-2 border-none !ring-0 w-full" value={getInputChapter} onChange={(e) => setInputChapter(e.target.value)} onKeyDown={handleEditChapter} />
                          <div onClick={() => setEditChapter(false)} className="absolute right-2.5 cursor-pointer hover:text-gray-700">
                            <RxCross2 />
                          </div>
                        </div>
                      ) : (
                        <div className="flex justify-between gap-2 items-center">
                          <div onClick={() => handleVideoPerChapter(value.chapter_id)} className="cursor-pointer hover:text-gray-400 whitespace-nowrap overflow-hidden">
                            {value.title}
                          </div>
                          <div className="flex gap-2 items-center">
                            <HiOutlinePencilAlt onClick={() => handleBtnEditChapter(value.chapter_id)} className="w-5 h-5 cursor-pointer text-yellow-400 hover:text-yellow-300" />
                            {/* <HiOutlineTrash
                              onClick={() =>
                ...
                {getAddChapter && (
                  <div className="relative flex items-center border-b border-gray-300">
                    <input type="text" placeholder="new chapter" className="py-2 border-none !ring-0 w-full" onChange={(e) => setInputChapter(e.target.value)} onKeyDown={handleAddChapter} />
                    <div onClick={() => setAddChapter(false)} className="absolute right-2.5 cursor-pointer hover:text-gray-700">
                      <RxCross2 />
                    </div>
                  </div>
                )}
                <div onClick={() => setAddChapter(!getAddChapter ? true : true)} className="flex items-center justify-center gap-2 w-fit mx-auto mt-2 px-3 py-1.5 rounded cursor-pointer bg-green-400 text-white hover:text-gray-300">
            ...
            <div className={`${getCloseAddEdit ? "w-1/4" : "w-1/2"}`}>
              <div className="flex gap-2 items-center bg-primary text-white rounded px-2 text-lg font-semibold">
                ...
                {getDataAllChapter[0]?.title ? (
                  <>
                    <BsDot />
                    <div className="overflow-hidden whitespace-nowrap">{getDataAllChapter[0]?.title}</div>
                  </>
                ) : (
                  ""
                )}
              ...
              <div className="mt-1 font-medium text-sm desktop:h-[470px] desktopfull:h-[800px] overflow-y-auto">
                <div className={`text-center py-2 ${getVideoChapter.length === 0 ? "block" : "hidden"}`}>Belum ada video.</div>
                <div onClick={() => handleAddVideo()} className={`${getIdChapter ? "flex" : "hidden"} items-center justify-center gap-2 w-fit mx-auto mt-2 px-3 py-1.5 rounded cursor-pointer bg-green-400 text-white hover:text-gray-300`}>
                ...
                {getVideoChapter.map((value, index) => {
                  return (
                    <div key={index} className="flex justify-between items-center gap-2 py-2 border-b border-gray-300">
                      <div className="whitespace-nowrap overflow-hidden">{value.title}</div>
                      <div className="flex gap-2 items-center">
                        <HiOutlinePencilAlt onClick={() => handleEditVideo(value.video_id)} className="w-5 h-5 cursor-pointer text-yellow-400 hover:text-yellow-300" />
                        <HiOutlineTrash onClick={() => handleDeleteVideo(value.video_id)} className="w-5 h-5 cursor-pointer text-red-400 hover:text-red-300" />
                      </div>
                    </div>
                  );
                })}
            ...
            <div className={`${getCloseAddEdit ? "w-1/2 block" : "hidden"}`}>
              <div className="flex justify-between items-center gap-1.5 bg-primary text-white rounded px-2 text-lg font-semibold">
                ...
                <RxCross2 className="cursor-pointer hover:text-gray-400" onClick={() => setCloseAddEdit(false)} />
              </div>
              <div className="mt-2.5 px-2 space-y-2 font-medium text-sm">
                <div className="flex flex-col gap-1">
                  <label htmlFor="judul">Judul</label>
                  <input type="text" placeholder="masukkan judul" id="judul" value={getTitleVideo} onChange={(e) => setTitleVideo(e.target.value)} className="w-full px-4 py-1.5 text-sm rounded" />
                </div>

                <div className="flex flex-col gap-1">
                  <label htmlFor="deskripsi">deskripsi</label>
                  <input type="text" placeholder="masukkan deskripsi" id="deskripsi" value={getDeskripsiVideo} onChange={(e) => setDeskripsiVideo(e.target.value)} className="w-full px-4 py-1.5 text-sm rounded" />
                </div>

                <div className="flex flex-col gap-1">
                  <label htmlFor="link">Link Video</label>
                  <input type="text" placeholder="masukkan link video" id="link" value={getUrlVideo} onChange={(e) => setUrlVideo(e.target.value)} className="w-full px-4 py-1.5 text-sm rounded" />
                </div>

                <div className="flex flex-col gap-1">
                  <label htmlFor="preview">Is_preview</label>
                  <select name="preview" id="preview" value={getPreviewVideo} onChange={(e) => setPreviewVideo(e.target.value === "true")} className="w-full px-4 py-1.5 text-sm rounded">
                ...
                <div className="flex justify-end gap-3">
                  {getAddEdit ? (
                    <button onClick={() => handleBtnSimpanVideo()} className="bg-primary text-white px-4 py-1.5 hover:text-gray-400 rounded">
                      Simpan
                    </button>
                  ) : (
                    <button onClick={() => handleBtnEditVideo()} className="bg-primary text-white px-4 py-1.5 hover:text-gray-400 rounded">
                      Ubah
                    </button>
                  )}
                </div>
