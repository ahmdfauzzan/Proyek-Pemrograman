...
export const DashboardAdmin = () => {
  ...
  }, [searchValue, getStatus, isSuccess, currentPage, getPayment]);
      ...
      <div className="absolute right-0 mt-2 bg-white shadow-lg border-t border-primary rounded-lg px-3 pt-1.5 pb-3 text-sm">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-1.5">
            <input type="radio" id="notyet" name="status" value="Belum Bayar" onChange={(e) => handleRadio(e.target.value)} checked={getStatus === "Belum Bayar"} className="h-4 w-4 border border-gray-300 rounded-md" />
            <label htmlFor="notyet" className="whitespace-nowrap">
              Belum Bayar
            </label>
          </div>
          <div className="flex items-center gap-1.5">
            <input type="radio" id="done" name="status" value="Sudah Bayar" onChange={(e) => handleRadio(e.target.value)} checked={getStatus === "Sudah Bayar"} className="h-4 w-4 border border-gray-300 rounded-md" />
        ...
        <div onClick={() => setStatus("")} className="py-0.5 text-center bg-slate-400 text-white rounded-lg cursor-pointer">
          Reset
        </div>
      </div>
    );
        ...
        <div className="flex flex-col gap-7">
          ...
          <div>
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-6 text-primary">
                <div className="relative">
                  <div className="flex items-center gap-1 text-lg border border-primary rounded-lg px-4 py-0.5 cursor-pointer hover:text-purple-800" onClick={() => (getModalFilter ? setModalFilter(false) : setModalFilter(true))}>
            ...
            <table className="text-sm w-full mt-3 text-left">
              ...
              <tbody>
                {dataPayment.map((value, index) => {
                  return (
                    <tr key={index}>
                      <td className="py-3 text-center">{value.riwayat_transaksi_id}</td>
            ...
            <div className="flex justify-center mt-8 mobile:w-[70%] desktop:w-full">
              ...
              <button
                className={`px-4 py-2 mx-1 rounded text-white font-bold ${currentPage === lastPage ? "bg-gray-300" : "bg-[#489CFF] cursor-pointer"}`}
